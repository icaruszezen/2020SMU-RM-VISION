...
void ArmorDetector::findContourInEnemyColor(
    ...
    while (it != contours_br.end()){
		...
		if (count_left > 10){
			...
			vector<Vec4i> hierarchy;
			findContours(left(rect), contour, hierarchy, RETR_LIST, CHAIN_APPROX_SIMPLE, cv::Point2i(rect.x, rect.y));
		...
		if (count_right > 10){
            vector<vector<Point2i> > contour;
			vector<Vec4i> hierarchy;
			findContours(right(rect), contour, hierarchy, RETR_LIST, CHAIN_APPROX_SIMPLE, cv::Point2i(rect.x, rect.y));
...
cv::RotatedRect ArmorDetector::chooseTarget(const std::vector<cv::RotatedRect> & rects, const std::vector<double> & score) {
    ...
    threshold(s_b, s_b, 128, 255, THRESH_OTSU);
    vector<vector<Point2i> > contours_br;
	vector<Vec4i> hierarchy;
    findContours(s_b, contours_br, hierarchy, RETR_EXTERNAL , CHAIN_APPROX_SIMPLE);
