...
void ImageConsProd::ImageProducer(){
    ...
    string video_name = "./data/test.mp4";
    ...
    cap.setExposureTime(0, 30);//settings->exposure_time);
    cap.startStream();
    cap.info();
#endif
...
void ImageConsProd::ImageConsumer(){
    ...
    if (setting.save_result > 0){      
        vw.open("webcam.mp4",VideoWriter::fourcc('M', 'J', 'P', 'G'), 30, cv::Size(VIDEO_WIDTH, VIDEO_HEIGHT), true);
        vw_src.open("webcam_src.mp4", VideoWriter::fourcc('M', 'J', 'P', 'G'), 30, cv::Size(VIDEO_WIDTH, VIDEO_HEIGHT), true);
    ...
    while(1){
        ...
        if(setting.show_image > 0 || setting.save_result > 0){
            ...
            putText(src_csm, str, Point(10, 40), FONT_HERSHEY_COMPLEX_SMALL, 1.3, CV_RGB(128, 255, 0), 1);
            t2 = cv::getTickCount();
            cout << "Consumer-Time: " << (t2 - t1) * 1000.0 / cv::getTickFrequency() << "ms,frame number:" << frame_num << endl;
        }
